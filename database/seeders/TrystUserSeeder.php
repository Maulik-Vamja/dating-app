   ...
   // protected $galaryImage = GalleryImages::class;
    ...
    public function run(): void
    {
        ...
        $user = User::create([
            ...
            'is_trans' => $userJson['trans'] === null ? 'n' : 'y',
...
if (isset($userJson['contact'])) {
   foreach ($userJson['contact'] as $type => $value) {
    $type = ucfirst($type); // Ensure $type is defined

    $contactMedia = ContactMedia::firstOrCreate(
        ['name' => $type]
    );
...
if (isset($userJson['based_in'])) {
    ...
    Log::info("based-id: {$cityName}x{$stateName}");
...
if (isset($userJson['locations']) && is_array($userJson['locations'])) {
    foreach ($userJson['locations'] as $location) {
        // Split the location into city and state
        $locationParts = explode('/', $location);

        $cityName = ucwords(str_replace('-', ' ', $locationParts[1] ?? null));
        $stateName = ucwords(str_replace('-', ' ', $locationParts[0] ?? null));


        // Find or create the State within the USA
        $state = State::firstOrCreate(['name' => $stateName, 'country_id' => 236]);

        // Find or create the City within the given State and USA
       $city = City::firstOrCreate(['name' => $cityName, 'state_id' => $state->id, 'country_id' => 236]);

        // Check if State and City were found or created
        if ($state->wasRecentlyCreated) {
            Log::info("State created: {$state->id} - {$state->name}");
        } else {
            Log::info("State found: {$state->id} - {$state->name}");
        }

        if ($city->wasRecentlyCreated) {
            Log::info("City created: {$city->id} - {$city->name}");
        } else {
            Log::info("City found: {$city->id} - {$city->name}");
        }

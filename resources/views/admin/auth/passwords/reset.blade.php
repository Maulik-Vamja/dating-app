            ...
            <div class="d-flex flex-center flex-row-fluid bgi-size-cover bgi-position-top bgi-no-repeat" style="background-image: url({{ asset('assets/media/bg/bg-3.jpg') }});">
                <div class="login-form text-center p-7 position-relative overflow-hidden" style="max-width: 450px;width:100%">
                    <div class="d-flex flex-center mb-15">
                        <a href="#">
                            <img class="img-fluid" style="max-width: 240px; max-height: 60px;" src="{{ asset($sitesetting['site_logo']) }}" alt="{{ env('APP_NAME') }}" />
                        </a>
                    </div>
                    <div class="login-signin">
                        ...
                        <form class="form-horizontal" id="resetpwdForm" role="form" method="POST" action="{{ url('/admin/password/reset') }}" class="form">
                            ...
                            <div class="form-group{{ $errors->has('email') ? ' has-error' : '' }}">
                                <input id="email" type="email" class="form-control h-auto py-4 px-8 bg-secondary" name="email" value="{{ $email ?? old('email') }}" required autocomplete="email" readonly placeholder="Email">
                                @if ($errors->has('email'))
                            ...
                            <div class="form-group{{ $errors->has('password') ? ' has-error' : '' }}">
                                <input id="password" type="password" class="form-control h-auto py-4 px-8" name="password" placeholder="Password"required autocomplete="new-password" autofocus>

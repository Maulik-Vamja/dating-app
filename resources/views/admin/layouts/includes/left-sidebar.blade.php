<div class="aside aside-left aside-fixed d-flex flex-column flex-row-auto" id="kt_aside">
    <div class="brand flex-column-auto" id="kt_brand">
        <a href="{{ route('admin.dashboard.index') }}" class="brand-logo">
            <img style="max-width: 100px; max-height: 50px; position: absolute; vertical-align: middle; margin: -19px 15px 15px 30px;"
                alt="{{ env('APP_NAME') }}"
                src="{{ generate_url($sitesetting['site_logo'], asset('frontend/images/logo.png')) }}" />
        </a>
    ...
    <div class="aside-menu-wrapper flex-column-fluid" id="kt_aside_menu_wrapper">
        <div id="kt_aside_menu" class="aside-menu my-4" data-menu-vertical="1" data-menu-scroll="1"
            ...
            <div class="ms-SearchBox">
                <input class="ms-SearchBox-field" type="text" value="" id="search" style="width: 265px;"
                    autocomplete="off" spellcheck="false" autocapitalize="off" autocorrect="off" dir="ltr" />
            ...
            <ul class="menu-nav">
                ...
                @foreach ($menu as $section)
                    ...
                    @if (count($section['roles']) > 1)
                        ...
                        @endphp
                        <li class="menu-item menu-item-submenu side-item {{ $activeSubmenu }}"
                            data-title="'.{{ strtolower($section['name']) }}.'" aria-haspopup="true"
                                ...
                                <ul class="menu-subnav">
                                    ...
                                    @foreach ($section['roles'] as $role)
                                        <li class="menu-item {{ $role['class'] }}" aria-haspopup="true">
                                            <a href="{{ !empty($role['params']) ? route($role['route'], $role['params']) : route($role['route']) }}"
                                                class="menu-link">
                                                @if ($role['icon_type'] == 'image')
                                                    <img class="menu-icon" src="#"
                                                        alt="{{ str_slug($role['title']) }}">
                                                @else
             ...
             <span></span></i> -->
                    ...
                    @else
                        ...
                        <li class="menu-item side-item {{ $menu['class'] }}" aria-haspopup="true"
                            data-title="'.{{ strtolower($menu['title']) }}.'">
                            <a href="{{ !empty($menu['params']) ? route($menu['route'], $menu['params']) : route($menu['route']) }}"
                                class="menu-link">

@php
    $required = $setting->required == 'y' ? ' required ' : '';
@endphp
...
@switch($setting->type)
    ...
    @case('radio')
        ...
        <div class="form-group form-md-line-input">
            <label
                class="col-lg-2 col-form-label text-right"
                for="form_control_1"
            >
                {{ $setting->label }}
            </label>
                    ...
                    @foreach ($options as $key => $value)
                        <label class="radio radio-solid">
                            <input
                                type="radio"
                                id="radio53_{{ $key }}"
                                @php echo $value == $setting->value ? 'checked' : ''; @endphp
                                data-error-container="#privacy_error_{{ $setting->id }}" value="{{ $value }}"
                                name="{{ $setting->id }}"
                                {{ $required }}
                            >
                            <span></span>{{ $value }}
            ...
            <div class="col-lg-9 col-xl-6">
                ...
                >
                    <div
                        ...
                        style="background-image:url({{ generate_url($setting->value, '//via.placeholder.com/120/EBF4FF/7F9CF5?text=Logo') }})"
                    ></div>
                    <label
